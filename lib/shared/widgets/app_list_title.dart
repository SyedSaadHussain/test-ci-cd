...
Widget AppListTitle({Function? onTab,String title="",String subTitle="",IconData? icon,
  Color iconColor= AppColors.primary,Widget? trailing}) {
  ...
  return ListTile(
    ...
    trailing: trailing??(onTab==null?null:Icon(
...
Widget AppListTitle2({Function? onTab,String title="",TextStyle? labelStyle, dynamic? subTitle="",IconData? icon,bool isDate=false,
               ...
               title: Column(
                 ...
                 children: [
                   title.isEmpty?Container():Row(
                     children: [
                       Expanded(child: Text(title??"",style:labelStyle??TextStyle(color: (isRequired??false)?Colors.red:Colors.grey,fontSize: 13,fontWeight: (isRequired??false)?FontWeight.normal:FontWeight.w300),)),
                   ...
                   GestureDetector(
                     ...
                     child: Container(
                       ...
                       child:  ClipRRect(
                         borderRadius: BorderRadius.circular(8.0),
                         child: Image.network(value,headers: headersMap,fit: BoxFit.cover,
  ...
  return  Container(
    ...
    child: ListTile(
      ...
      Container(
        height: 60,
        width: 60,
        child: emp.id==null?Container():Image.network('${baseUrl}/web/image?model=hr.employee&field=avatar_128&id=${emp.id}&unique=${""}',headers: headersMap,fit: BoxFit.fitHeight,
          errorBuilder: (BuildContext context, Object error, StackTrace? stackTrace) {
            // This function will be called when the image fails to load
            return Padding(
              padding: const EdgeInsets.all(8.0),
...
Widget EmployeeCard(Employee emp,dynamic headersMap,String baseUrl,{Function? onTab,Function? onDelete,bool isEditMode=false}) {
  ...
  return  Container(
    ...
    child: ListTile(
      ...
      title: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(emp.name??'',style: AppTextStyles.cardTitle,),
          Text(emp.classification??'',style: AppTextStyles.cardTitle,),
        ],
      ),
      // subtitle: Text(emp.title??"",style: TextStyle(color:Colors.grey),),
    ),
...
class AppBoolean extends StatelessWidget {
  ...
  AppBoolean({this.value,this.label});
      ...
      child: Wrap(
        children: [
          value!?Icon(
            ...
            color: Colors.green.withOpacity(.8), // Set the color to green
            size: 16,
          ):Icon(
            FontAwesomeIcons.minusCircle,
            color: Colors.grey.withOpacity(.9), // Set the color to red
            size: 16,
          ),
          Container(
              margin: EdgeInsets.symmetric(vertical: 0,horizontal: 2),
              child: Text(this.label??"",style: AppTextStyles.formLabel,)),
        ],
      ),
    );
  }
...
class AppHint extends StatelessWidget {
  ...
  AppHint({this.text,this.isHide});

  @override
  Widget build(BuildContext context) {
    return ((this.isHide??false) || ((text??"").trim()==''))?Container():Container(

      padding: EdgeInsets.symmetric(vertical: 2,horizontal: 5),
      child: Text((text??""),style: AppTextStyles.hint,),
    );
  }
}