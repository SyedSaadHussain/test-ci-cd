...
class MosqueUserList extends StatefulWidget {
  ...
  final List<int>? ids;
  MosqueUserList({required this.client,required this.onItemTap,this.type,this.title,this.onAddEmployee,this.supervisorId,this.ids});
  @override
  _MosqueUserListState createState() => _MosqueUserListState();
}

class _MosqueUserListState extends State<MosqueUserList> {
  ...
  void searchRecords(bool isReload){
    ...
    _userService!.getMosqueUser(10,data.pageIndex,_controller.text,this.widget.type??"",this.widget.supervisorId,ids: this.widget.ids).then((value){
    ...
    return Container(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ModalTitle(this.widget.title??"",FontAwesomeIcons.users,leading:
                ...
                itemBuilder: (BuildContext context, int index) {
                  if(index >= data.list!.length)
                  {
                    ...
                    return Container(
                      ...
                      child: ListTile(
                        leading: ImageData(id:data.list[index].id!,modelName: Model.employee,fieldId: "avatar_128",width: 30,height: 40,
                          uniqueId: data.list[index].uniqueId,baseUrl: userProvider.baseUrl,headersMap:_userService==null?'':_headersMap ,),
                        ...
                        title: Column(
                          ...
                          children: [
                            ...
                            Text(data.list[index].name??"",style: TextStyle(fontWeight: FontWeight.w400,color: AppColors.bodyText),),
                            Text(data.list[index].identificationId??"",style: TextStyle(fontWeight: FontWeight.w300,color: AppColors.body.withOpacity(.5),fontSize: 14),),
                          ],
                        ),
