...
class MultiItemList extends StatefulWidget {
  final CustomOdooClient client;
  final List<ComboItem> items;
  final Function onItemTap;
  final List<dynamic>? selectedValue;
  final String? title;
  MultiItemList({required this.client,required this.onItemTap,required this.items,this.title,this.selectedValue});
  @override
  _MultiItemListState createState() => _MultiItemListState();
}

class _MultiItemListState extends State<MultiItemList> {
            ...
            Expanded(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return  Opacity(
                    opacity: (this.widget.selectedValue??[])!.contains(this.widget.items[index].key)?(.5):1,
                    ...
                    child: Container(
                      ...
                      child: ListTile(
                        ...
                        onTap:(this.widget.selectedValue??[])!.contains(this.widget.items[index].key)?null: () {
                          // Perform actions when an item is tapped
                          this.widget.onItemTap(this.widget.items[index]);
                        },
                        trailing: (this.widget.selectedValue??[])!.contains(this.widget.items[index].key)?Icon(Icons.check_circle,color: Colors.lightGreen.shade400,):null,
                      ),
                    ),
                  );
                },
