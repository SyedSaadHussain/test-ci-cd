...
class ComboBoxList extends StatefulWidget {
  ...
  final String? source; // ✅ 'classification' or 'mosque_type'

  ComboBoxList({required this.client,required this.onItemTap,this.label, this.source});
  @override
...
class _ComboBoxListState extends State<ComboBoxList> {
  ...
  void searchRecords() {
    ...
    if (widget.source == 'classification') {
    ...
    } else if (widget.source == 'mosque_type') {
      _userService!.getMosqueType().then((value) {
        setState(() {
          data.list = value;
          data.isloading = false;
        });
