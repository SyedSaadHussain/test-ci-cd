...
import '../../../electricity_consumption/ui/widgets/filter_dropdown_widget.dart';
...
class FiltersSectionWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<WaterFilterProvider>(
                ...
                children: [
                  Expanded(
                    child: FilterDropdownWidget(
                      label: 'region'.tr(),
                      selectedValue: filterProvider.getRegionById(filterProvider.selectedRegionId ?? 0),
                      ...
                      onChanged: (region) => filterProvider.selectRegion(region?.id),
                    ),
                  ),
                  ...
                  Expanded(
                    child: FilterDropdownWidget(
                      label: 'city'.tr(),
                      selectedValue: filterProvider.getCityById(filterProvider.selectedCityId ?? 0),
                      ...
                      onChanged: (city) => filterProvider.selectCity(city?.id),
                    ),
                  ),
                ],
              ),
              ...
              Row(
                children: [
                  Expanded(
                    child: FilterDropdownWidget(
                      label: 'center'.tr(),
                      selectedValue: filterProvider.getCenterById(filterProvider.selectedCenterId ?? 0),
                      items: filterProvider.availableCenters,
                      isEnabled: filterProvider.canSelectCenter,
                      isLoading: filterProvider.isLoadingCenters,
                      onChanged: (center) => filterProvider.selectCenter(center?.id),
                    ),
                  ),
                ],
              ),
