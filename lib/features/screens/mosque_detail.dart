...
import 'package:mosque_management_system/shared/widgets/list/item_list.dart';
import 'package:mosque_management_system/shared/widgets/list/meter_form.dart';
...
import 'package:mosque_management_system/shared/widgets/list/partner_list.dart';
...
class MosqueDetail extends StatefulWidget {
  ...
  MosqueDetail({required this.mosqueId,this.onCallback});
  @override
...
class _MosqueDetailState extends BaseState<MosqueDetail> with SingleTickerProviderStateMixin  {
  ...
  List<Employee> _relateEmployeeIds=[];
  ...
  List<MosqueMeter> _allMosqueMeters=[];
  List<MosqueMeter> _tempAllMosqueMeters=[];
  List<MosqueMeter> _allWaterMeters=[];
  List<MosqueMeter> _tempAllWaterMeters=[];
  List<TabForm> _tabs=[];
  bool isLoading=true;
  List<ComboItem> instrumentEntities=[];
  List<ComboItem> buildingTypeIds=[];
  late TabController _tabController = TabController(length: 9, vsync: this);
  @override
  void dispose() {
  ...
  void widgetInitialized() {
    ...
    _userService!.getHeadersMap().then((headerMap){_headersMap=headerMap;setState(() {
  ...
  void renderTabs(){
    ...
    _tabs.add(TabForm(title: 'meters'.tr(),isShowEdit:true));
    ...
    //_tabs.add(TabForm(title: 'employee_info'.tr(),isShowEdit:false));
  ...
  bool isVerifyDevice(){
    //return true;//urgent
    if(appUserProvider.isDeviceVerify || Config.disableValidation) return true;
    else{
      Flushbar(
        icon: Icon(Icons.warning,color: Colors.white,),
        backgroundColor: AppColors.danger,
  ...
  void getAllEmployees(){
    ...
    uniqueEmployeeIds.addAll(_mosque.relateEmployeeIds??[]);
    ...
    if(employeeIds.length>0){
      ...
      _userService!.getEmployeesByIds(employeeIds).then((employees){
        ...
        _relateEmployeeIds=_allEmployees.where((emp) {
          return  _mosque.relateEmployeeIds!.contains(emp.id);
        }).toList();
  ...
  void getMeters(){
    ...
    meterIds.addAll(_mosque.meterIds??[]);
    waterMeterIds.addAll(_mosque.waterMeterIds??[]);
    print('getMeters');
    print(_mosque.waterMeterIds);
    setState((){});
    print(meterIds.length);
    if(meterIds.length>0){
      _mosqueService!.getMosqueMetersByIds(meterIds).then((data){
        _allMosqueMeters=data;
        print('getMeters');
        print(_allMosqueMeters);
        setState(() {
    ...
    if(waterMeterIds.length>0){
      _mosqueService!.getWaterMetersByIds(waterMeterIds).then((data){
        _allWaterMeters=data;
        print('getMeters');
        print(_allWaterMeters);
        setState(() {
  ...
  /// Automatically calculates capacity for men's prayer section based on formula:
  /// capacity = row_men_praying_number * length_row_men_praying * 2
  void _calculateMenPrayerCapacity() {
    final rowNumber = _mosquePrayerSection.rowMenPrayingNumber;
    final rowLength = _mosquePrayerSection.lengthRowMenPraying;
    
    if (rowNumber != null && rowNumber > 0 && rowLength != null && rowLength > 0) {
      _mosquePrayerSection.capacity = (rowNumber * rowLength * 2).round();
    }
  }
  ...
  void _calculateWomenPrayerCapacity() {
    final rowNumber = _mosquePrayerSection.rowWomenPrayingNumber;
    final rowLength = _mosquePrayerSection.lengthRowWomenPraying;
    
    if (rowNumber != null && rowNumber > 0 && rowLength != null && rowLength > 0) {
      _mosquePrayerSection.womenPrayerRoomCapacity = (rowNumber * rowLength * 2).round();
    }
  }
...
//#endregion  Get Methods
      ...
      builder: (BuildContext context) {
        return ComboBoxList(client: appUserProvider.client!,
          ...
          onItemTap: (ComboItem val){
            ...
            _mosqueBasicInfo.mosqueType=val.value;
            _mosqueBasicInfo.mosqueTypeId=val.key;
  ...
  updateMeters(index){
    ...
    _mosqueService!.updateMosqueMeters(_mosque,_tempAllMosqueMeters,_tempAllWaterMeters).then((value){
      setState((){
        _tabs[index].isEditMode=false;
        isLoading=false;
      });
      _allMosqueMeters=[];
      _allWaterMeters=[];
      _tempAllMosqueMeters.forEach((meter) {
        _allMosqueMeters.add(MosqueMeter.shallowCopy(meter));
      });
      _tempAllWaterMeters.forEach((meter) {
        _allWaterMeters.add(MosqueMeter.shallowCopy(meter));
      });
      getMosqueDetail();
    }).catchError((e){
      setState((){
  ...
  AddEditMeterModal({Meter? item=null,Function? onSave,String title='',String? labelName,bool isAttachment=false,bool isShared=false}){
    ...
    showModalBottomSheet(
      ...
      context: context,
      builder: (BuildContext context) {
        return MeterFrom(client: appUserProvider.client!,labelName:labelName,title:title,
  ...
  AddEditMosqueMeterModal({MosqueMeter? item=null,Function? onSave,String title='',String? labelName,bool isAttachment=false,bool isShared=false,String? meterType}){

   print(fields.getField("meter_ids").child!.list.length);
    MosqueMeter _meter=item??MosqueMeter(id: 0,meterType: meterType);
    showModalBottomSheet(
      isScrollControlled: true,
      context: context,
      builder: (BuildContext context) {
        return MosqueMeterFrom(client: appUserProvider.client!,labelName:labelName,title:title,
          item:_meter,mosque: _mosque,
          headersMap:  _headersMap,
          onSave: onSave,
          isShared: isShared,
          fields: fields.getField("meter_ids").child,
          isAttachment: isAttachment,
        );
      },
    );
  ...
  void setDraftMosque(){
    ...
    _mosqueService.setDraftMosque(_mosque.id!).then((response){


      setState(() {
        isLoading=false;
      });
      _mosqueService.getSetDraftDisclaimer(_mosque.id??0).then((result){
        disclaimer=result;
        showDisclaimerDialogWithReason(context,text:(JsonUtils.toText(disclaimer["value"]["text"])??""),onApproved: (reason){
          acceptTermsSetDraft(reason);
        });
  ...
  void acceptTermsSetDraft(String refuseText){
    setState(() {
      isLoading=true;
    });
    _mosqueService.acceptTermsSetDraft(_mosque.id,refuseText).then((_){
      if(this.widget.onCallback!=null)
        this.widget.onCallback!();
      getMosqueDetail();
      setState(() {
  ...
  void submitMosqueChanges(){
    setState(() {
      isLoading=true;
    });
    _mosqueService.submitMosqueChanges(_mosque.id!).then((response){
  ...
  showRefuseMosqueTerms(){
    _mosqueService.getRefuseMosqueDisclaimer(_mosque.id??0).then((result){
      disclaimer=result;
      showDisclaimerDialogWithReason(context,text:(JsonUtils.toText(disclaimer["value"]["text"])??""),onApproved: (reason){
        refuseMosqueTerms(reason);
      },isRequired: true,fieldLabel: 'refuse_reason'.tr());
    }).catchError((e){

    });
  }
  void refuseMosqueTerms(String refuseText){
    setState(() {
      isLoading=true;
    });
    _mosqueService.refuseMosqueTerms(_mosque.id,refuseText).then((_){
      if(this.widget.onCallback!=null)
        this.widget.onCallback!();
      getMosqueDetail();
      setState(() {
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      (_mosque!.displayButtonSetToDraft??false)!?Container(
                        padding: EdgeInsets.symmetric(horizontal: 3,vertical: 0),

                        child: AppButton(text: "set_to_draft".tr(),onTab:(){
                          if(isVerifyDevice()) {
                            //confirmMosque();
                            setDraftMosque();
                          }
                        },icon: FontAwesomeIcons.rotateLeft,color:AppColors.gray),
                      ):Container(),
                      ...
                      (_mosque!.isEditMosqueMode??false)!?Container(
                        padding: EdgeInsets.symmetric(horizontal: 3,vertical: 0),

                        child: DangerButton(text: "submit_changes".tr(),onTab:(){
                          if(isVerifyDevice()) {
                            //confirmMosque();
                            submitMosqueChanges();
                          }
                      ...
                      children: [
                        GestureDetector(
                           ...
                           Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) => FullScreenImagePage(
                                  title :_mosque.name??"",
                                  imageUrl: '${appUserProvider.baseUrl}/web/image?model=mosque.mosque&field=outer_image&id=${_mosque.id}&unique=${_mosque.uniqueId}',
                                ),
                              ),
                            );
                          },
                          child: Container(
                            ...
                            // ),
                            child:  ClipRRect(
                              child: Image.network('${appUserProvider.baseUrl}/web/image?model=mosque.mosque&field=outer_image&id=${_mosque.id}&unique=${_mosque.uniqueId}',headers: _headersMap??null,
                                        ...
                                        _tabs[_tabController.index].isEditMode?Column(
                                          children: [
                                                    ...
                                                    child: Column(
                                                      children: [
                                                        ...
                                                        // CustomFormField(title: 'Number',value: _mosqueDetail.number,isRequired: true,onChanged:(val) => _mosqueDetail.number = val,),
                                                         ...
                                                         _mosqueBasicInfo.isEditMosqueMode?Column(
                                                          children:[
                                                            AppListTitle2(title: fields.getField('region_id').label,subTitle: _mosqueBasicInfo.region??'',isRequired:requiredFields.contains("region_id")),
                                                        ...
                                                        /// 🟦 Mosque Type ID - Optional
                                                        // CustomFormField(
                                                        //   title: fields.getField('mosque_type_id').label,
                                                        //   isRequiredField: false, //
                                                        //   onTab: () {
                                                        //     showMosqueTypeModal();
                                                        //     print("🟦 myMosque Type: ${_mosqueBasicInfo.mosqueType}");
                                                        //
                                                        //   },
                                                        //
                                                        //   value: _mosqueBasicInfo.mosqueType,
                                                        //
                                                        //   isReadonly: true,
                                                        //   isSelection: true,
                                                        // ),
                                                        ...
                                                        _mosqueBasicInfo.isEditMosqueMode?Column(children:[
                                                                  ...
                                                                  return MosqueUserList(client: appUserProvider.client!,
                                                                    ...
                                                                    onAddEmployee: (){
                                                                      ...
                                                                      Navigator.push(
                                                                        context,
                                                                        MaterialPageRoute(
                                                                          builder: (context) => new CreateEmployee(mosqueId: this.widget.mosqueId,
                                                                          type: "imam",
                                                                          onCreated:(Employee newEmployee){

                                                                              _imams.add(newEmployee);
                                                                              setState(() {
                                                        ...
                                                        _mosqueBasicInfo.landOwner=="private"?Column(
                                                          children: [
                                                            CustomFormField(title: fields.getField('mosque_owner_name').label,value: _mosqueBasicInfo.mosqueOwnerName,
                                                                isRequiredField:requiredFields.contains("mosque_owner_name"),
                                                                onChanged:(val) => _mosqueBasicInfo.mosqueOwnerName = val),
                                                            CustomFormField(title: fields.getField('honor_name').label,value: _mosqueBasicInfo.honorName,
                                                                isRequiredField:requiredFields.contains("honor_name"),
                                                                onChanged:(val) => _mosqueBasicInfo.honorName = val),

                                                          ],
                                                        ):Container(),

                                                        AppUtils.isNotNullOrEmpty(_mosque.observationText)?
                                                        AppListTitle2(title: 'reason_for_return'.tr(),subTitle: _mosque.observationText,hasDivider:true):Container(),
                                                        AppUtils.isNotNullOrEmpty(_mosque.refuseReason)?
                                                        AppListTitle2(title: fields.getField('refuse_reason').label,subTitle: _mosque.refuseReason,hasDivider:true):Container(),
                                            ...
                                            Row(
                                              children: [
                                                Expanded(
                                                  child: PrimaryButton(text: "save_update".tr(),onTab:(){
                                          ...
                                          child: Container(
                                            ...
                                            child: Column(
                                              children: [
                                                ...
                                                AppListTitle2(title: fields.getField('old_sequence').label,subTitle: _mosque.oldSequence??'',isRequired:requiredFields.contains("region_id")),
                                                ...
                                                AppListTitle2(title: fields.getField('mosque_type_id').label,subTitle: _mosque.mosqueType??'',isRequired:requiredFields.contains("mosque_type_id")),
                                                ...
                                                _mosque.landOwner=="private"?Column(
                                                  children: [
                                                    AppListTitle2(title: fields.getField('mosque_owner_name').label,isRequired:requiredFields.contains("mosque_owner_name"),subTitle: _mosque.mosqueOwnerName??""),
                                          ...
                                          children: [
                                                  ...
                                                  child: Container(
                                                      ...
                                                      child: Column(
                                                        children: [
                                                          ...
                                                          AppTitle1('electric_meter'.tr(),Icons.power),

                                                          CustomListField(title: ' ',

                                                              buttonIcon : Icons.add,
                                                              buttonLabel : "add".tr(),

                                                              employees:  _tempAllMosqueMeters.where((item) => item.isDelete==false && item.meterType=="electric").toList(),isEditMode:_tabs[_tabController.index].isEditMode,
                                                              builder: (MosqueMeter item){
                                                                return  Container(
                                                                  ...
                                                                  child: Row(
                                                                    ...
                                                                    mainAxisAlignment: MainAxisAlignment.start,
                                                                    children: [
                                                                      ImageData(id: item.id,modelName: Model.mosqueMeter,width: 60,height: 70,uniqueId:item.uniqueId,baseUrl: appUserProvider.baseUrl,headersMap:_headersMap??'' ,fieldId: "attachment_id",),
                                                                          ...
                                                                          child: Column(
                                                                            ...
                                                                            children: [
                                                                              ...
                                                                              Row(
                                                                                ...
                                                                                children: [
                                                                                  Expanded(
                                                                                    child: Text(item.meterNumber!,style: TextStyle(color: AppColors.defaultColor),
                                                                                      overflow: TextOverflow.visible,
                                                                                    ),
                                                                                  ),

                                                                                  (item!.meterNew??false)?AppTagSm(title: 'new'.tr(),color: AppColors.success):Container(),
                                                                                ],
                                                                              ),
                                                                              Wrap(
                                                                                children: [
                                                                                  Row(
                                                                                    children: [
                                                                                      Expanded(
                                                                                          ...
                                                                                          child: AppBoolean(value: (item.imam??false),label:fields.getField('meter_ids').child!.getField("imam").label ,)),
                                                                                      Expanded(
                                                                                          child: AppBoolean(value: (item.muezzin??false),label:fields.getField('meter_ids').child!.getField("muezzin").label ,)),
                                                                                    ],
                                                                                  ),
                                                                                  Row(
                                                                                    children: [
                                                                                      Expanded(child: AppBoolean(value: (item.mosque??false),label:fields.getField('meter_ids').child!.getField("mosque").label ,)),
                                                                                      Expanded(child: AppBoolean(value: (item.facility??false),label:fields.getField('meter_ids').child!.getField("facility").label ,)),
                                                                              ...
                                                                              //       Row(
                                                                              //         children: [
                                                                              //           AppBoolean(value: (item.imam??false),),
                                                                              //           Text("imam".tr(),style: AppTextStyles.formLabel,),
                                                                              //         ],
                                                                              //       ),
                                                                              //       Row(
                                                                              //         children: [
                                                              ...
                                                              onEdit: (MosqueMeter item){

                                                                MosqueMeter copyItem=MosqueMeter.shallowCopy(item);
                                                                AddEditMosqueMeterModal(item: copyItem,
                                                                    title: 'electric_meter'.tr(),
                                                                    labelName: "electric_meter_number".tr(),
                                                                    meterType:item.meterType ,
                                                                    isAttachment: true,
                                                                    isShared: true,
                                                                    onSave:(MosqueMeter item){
                                                                      MosqueMeter itemToUpdate = _tempAllMosqueMeters.firstWhere((record) => record.id == item.id);

                                                                      itemToUpdate.isEdit = true;
                                                                      itemToUpdate.attachmentId = item.attachmentId;
                                                                      itemToUpdate.meterNumber = item.meterNumber;
                                                                      itemToUpdate.mosque = item.mosque;
                                                                      itemToUpdate.muezzin = item.muezzin;
                                                                      itemToUpdate.facility = item.facility;
                                                                      itemToUpdate.meterNew = item.meterNew;
                                                                      itemToUpdate.imam = item.imam;
                                                              ...
                                                              onTab:(){
                                                                ...
                                                                AddEditMosqueMeterModal(
                                                                    ...
                                                                    isShared: true,
                                                                    onSave:(MosqueMeter item){
                                                                      _tempAllMosqueMeters.add(MosqueMeter.shallowCopy(item));
                                                                      setState(() {
                                                          ...
                                                          AppTitle1('water_meter'.tr(),Icons.water),
                                                          ...
                                                          CustomListField(title: ' ',
                                                              ...
                                                              employees: _tempAllWaterMeters.where((item) => item.isDelete==false && item.meterType=="water").toList(),isEditMode:_tabs[_tabController.index].isEditMode,
                                                              builder: (MosqueMeter item){
                                                                return  Container(
                                                                  padding: EdgeInsets.symmetric(horizontal: 3),
                                                                  child: Row(
                                                                    ...
                                                                    mainAxisAlignment: MainAxisAlignment.start,
                                                                    children: [
                                                                      ImageData(id: item.id,modelName: Model.waterMeter,width: 60,height: 70,uniqueId:item.uniqueId,baseUrl: appUserProvider.baseUrl,headersMap:_headersMap??'' ,fieldId: "attachment_id",),
                                                                      ...
                                                                      Expanded(
                                                                        child: Container(
                                                                          child: Column(
                                                                            ...
                                                                            children: [
                                                                              // Text(item.attachmentId.toString()),
                                                                              Text(item.meterNumber!,style: TextStyle(color: AppColors.defaultColor),),
                                                                              Wrap(
                                                                                children: [
                                                                                  Row(
                                                                                    children: [
                                                              ...
                                                              onEdit: (MosqueMeter item){
                                                                MosqueMeter copyItem=MosqueMeter.shallowCopy(item);
                                                                AddEditMosqueMeterModal(item: copyItem,
                                                                    title: 'water_meter'.tr(),
                                                                    labelName: "electric_meter_number".tr(),
                                                                    meterType:item.meterType ,
                                                                    isAttachment: true,
                                                                    isShared: true,
                                                                    onSave:(MosqueMeter item){
                                                                      MosqueMeter itemToUpdate = _tempAllWaterMeters.firstWhere((record) => record.id == item.id);
                                                              ...
                                                              onTab:(){
                                                                ...
                                                                AddEditMosqueMeterModal(
                                                                    title: "water_meter".tr(),
                                                                    labelName: "Meter Name",
                                                                    meterType:"water",
                                                                    isAttachment: true,
                                                                    isShared: true,
                                                                    onSave:(MosqueMeter item){
                                                                      _tempAllWaterMeters.add(MosqueMeter.shallowCopy(item));
                                                                      setState(() {
                                            ...
                                            Row(
                                              children: [
                                                Expanded(
                                                  child: PrimaryButton(text: "save_update".tr(),onTab:(){
                                                    updateMeters(_tabController.index);
                                                  }),
                                                ),
                                                Expanded(
                                                  child: SecondaryButton(text: "cancel".tr(),onTab:(){
                                                ...
                                                child: Column(
                                                  children: [
                                                    ...
                                                    CustomListField(title: '',
                                                        ...
                                                        employees:  _allMosqueMeters.where((item) => item.isDelete==false && item.meterType=="electric").toList(),isEditMode:_tabs[_tabController.index].isEditMode,
                                                        builder: (MosqueMeter item){
                                                          return  Container(
                                                            padding: EdgeInsets.symmetric(horizontal: 3),
                                                            child: Row(
                                                    ...
                                                    CustomListField(title: '',
                                                        ...
                                                        employees:  _allWaterMeters.where((item) => item.isDelete==false && item.meterType=="water").toList(),isEditMode:_tabs[_tabController.index].isEditMode,
                                                        builder: (MosqueMeter item){
                                                          return  Container(
                                                            padding: EdgeInsets.symmetric(horizontal: 3),
                                                            child: Row(
                                                  ...
                                                  child: Form(
                                                    ...
                                                    child: Column(
                                                      children: [
                                                        ...
                                                        _mosqueDetail.isQrCodeExist=='yes'?Column(
                                                          children: [
                                                            ...
                                                            CustomFormField(title: fields.getField('qr_image').label,isRequiredField:requiredFields.contains("qr_image"),
                                                                value: _mosqueDetail.qrImage==null?'':'${appUserProvider.baseUrl}/web/image?model=mosque.mosque&id=${_mosque.id}&field=qr_image&unique=${_mosque.uniqueId}',
                                                                onChanged:(val) {
                                                                  _mosqueDetail
                                                                      .qrImage =
                                                                      val;
                                                        ...
                                                        //           return  Row(
                                                        //             children: [
                                                        //               ImageData(id: item.id,modelName: Model.meter,width: 50,height: 50,uniqueId: item.uniqueId,baseUrl: appUserProvider.baseUrl,headersMap:_headersMap??'' ,fieldId: "attachment_id",),
                                                        //             
                                                        //               Column(
                                                        //                 crossAxisAlignment: CrossAxisAlignment.start,
                                                        //                 mainAxisAlignment: MainAxisAlignment.start,
                                                        ...
                                                        CustomFormField(title: fields.getField('mosque_size').label,value: _mosqueDetail.mosqueSize.toString(),onChanged:(val) => _mosqueDetail.mosqueSize = val,
                                                          isRequiredField:requiredFields.contains("mosque_size"),
                                                          type: FieldType.double,
                                                        ),
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: Column(
                                                      children: [
                                                        ...
                                                        _mosque.isQrCodeExist=='yes'?Column(
                                                          children: [
                                                            ...
                                                            AppListTitle2(title: fields.getField('qr_image').label,isRequired:requiredFields.contains("qr_image"),
                                                                subTitle: '${appUserProvider.baseUrl}/web/image?model=mosque.mosque&field=qr_image&id=${_mosque.id}&unique=${_mosque.uniqueId}'
                                                                ,type:ListType.image,headersMap:_headersMap??'',
                                                                onTab:(){
                                                                  Navigator.push(
                                                                    context,
                                                                    MaterialPageRoute(
                                                                      builder: (context) => FullScreenImagePage(
                                                                        ...
                                                                        imageUrl: '${appUserProvider.baseUrl}/web/image?model=mosque.mosque&field=qr_image&id=${_mosque.id}&unique=${_mosque.uniqueId}',
                                                                      ),
                                                                    ),
                                                                  );
                                                                }
                                                        ...
                                                        //                 baseUrl: appUserProvider.baseUrl,headersMap:_headersMap??'' ,
                                                        ...
                                                        AppListTitle2(title: fields.getField('mosque_size').label,subTitle: (_mosque.mosqueSize??'').toString()),
                                                    ...
                                                    child: Column(
                                                      children: [
                                                        ...
                                                        CustomFormField(title: fields.getField('image').label,
                                                            ...
                                                            value: _mosqueMosqueInfo.image==null?'':'${appUserProvider.baseUrl}/web/image?model=mosque.mosque&id=${_mosque.id}&field=image&unique=${_mosque.uniqueId}',
                                                            onChanged:(val) {
                                                        ...
                                                        CustomFormField(title: fields.getField('outer_image').label,
                                                            ...
                                                            value: _mosqueMosqueInfo.outerImage==null?'':'${appUserProvider.baseUrl}/web/image?model=mosque.mosque&id=${_mosque.id}&field=outer_image&unique=${_mosque.uniqueId}',
                                                          onChanged:(val) {
                                                            _mosqueMosqueInfo
                                                                .outerImage =
                                                                val;
                                                    ...
                                                    child: Column(
                                                      children: [
                                                        ...
                                                        AppListTitle2(title: fields.getField('image').label,isRequired:requiredFields.contains("image"),
                                                            subTitle: '${appUserProvider.baseUrl}/web/image?model=mosque.mosque&id=${_mosque.id}&field=image&unique=${_mosque.uniqueId}'
                                                            ,type:ListType.image,headersMap: _headersMap??null,
                                                             onTab:(){
                                                                ...
                                                                Navigator.push(
                                                                  ...
                                                                  MaterialPageRoute(
                                                                    builder: (context) => FullScreenImagePage(
                                                                      ...
                                                                      imageUrl: '${appUserProvider.baseUrl}/web/image?model=mosque.mosque&id=${_mosque.id}&field=image&unique=${_mosque.uniqueId}',
                                                                    ),
                                                                  ),
                                                                );
                                                            }
                                                        ...
                                                        AppListTitle2(title: fields.getField('outer_image').label,isRequired:requiredFields.contains("outer_image"),
                                                            subTitle: '${appUserProvider.baseUrl}/web/image?model=mosque.mosque&field=outer_image&id=${_mosque.id}&unique=${_mosque.uniqueId}'
                                                            ,type:ListType.image,headersMap: _headersMap??null,
                                                            onTab:(){
                                                              Navigator.push(
                                                                context,
                                                  ...
                                                  child: Form(
                                                    ...
                                                    child: Column(
                                                      children: [
                                                        ...
                                                        AppListTitle2(title: fields.getField('is_there_land_title').label,subTitle: _mosque.isThereLandTitle??''),
...
CustomFormField(title:  fields.getField('is_there_structure_buildings').label,value: _mosqueBuildingDetail.isThereStructureBuildings,
                                                        ...
                                                        _mosqueBuildingDetail.isOtherAttachment=='yes'?
                                              ...
                                              child: Container(
                                                  ...
                                                  child: Column(
                                                    children: [
                                                      ...
                                                      _mosque.isOtherAttachment=='yes'?
                                                      AppListTitle2(title: fields.getField('other_attachment').label,isRequired:requiredFields.contains("other_attachment"),subTitle: _mosque.otherAttachment??''):Container(),
                                                  ...
                                                  child: Form(
                                                    ...
                                                    child: Column(
                                                      children: [
                                                        ...
                                                        //               title: 'warning'.tr(),
                                                        //               message: "muezzin_meter_warning".tr(),
                                                        //               duration: Duration(seconds: 3),
                                                        //             ).show(context);
                                                  ...
                                                  child: Form(
                                                    ...
                                                    child: Column(
                                                      children: [
                                                          ...
                                                          children: [
                                                            CustomFormField(title: fields.getField('row_women_praying_number').label,value: _mosquePrayerSection.rowWomenPrayingNumber,
                                                                ...
                                                                onChanged:(val){
                                                                  ...
                                                                  _calculateWomenPrayerCapacity(); // Auto-calculate capacity
                                                                  setState(() {});
                                                                },type:FieldType.number),
                                                        ...
                                                        _mosquePrayerSection.classificationId==2?Container():CustomFormField(title: fields.getField('friday_prayer_rows').label,value: _mosquePrayerSection.fridayPrayerRows,
                                                        ...
                                                        CustomFormField(title:fields.getField('row_men_praying_number').label,value: _mosquePrayerSection.rowMenPrayingNumber,
                                                            ...
                                                            onChanged:(val){
                                                              ...
                                                              _calculateMenPrayerCapacity(); // Auto-calculate capacity
                                                              setState(() {});
                                                            } ,type:FieldType.number),
                                              ...
                                              children: [
                                                ...
                                                _mosque.hasWomenPrayerRoom=='yes'?Column(
                                                  children: [
                                                    ...
                                                    AppListTitle2(title: fields.getField('women_prayer_room_capacity').label,subTitle: _mosque.womenPrayerRoomCapacity??""),
                                                ...
                                                _mosque.classificationId==2?Container():AppListTitle2(title: fields.getField('friday_prayer_rows').label,isRequired:requiredFields.contains("friday_prayer_rows"),subTitle: (_mosque.fridayPrayerRows??'').toString()),
                                        ...
                                        // //#region Employees info
                                        // Container(
                                        //   padding: EdgeInsets.symmetric(horizontal: 10, vertical: 20),
                                        //   child: ListView.builder(
                                        //     itemCount: _relateEmployeeIds.length,
                                        ...
                                        //           child: Row(
                                        //             children: [
                                        //               ImageCircle(id: _relateEmployeeIds[index].id??0, modelName: Model.employee, fieldId: "avatar_128",uniqueId: "",baseUrl: appUserProvider.baseUrl,headersMap: _headersMap,height: 50,width: 50,),
                                        //
                                        //               Expanded(child:Container(
                                        ...
                                        //                   mainAxisAlignment: MainAxisAlignment.start,
                                        //                   children: [
                                        //                     Text(_relateEmployeeIds[index].name??'',style: AppTextStyles.cardTitle,),
                                        //                     Row(
                                        //                       children: [
                                        //                         Text("saad",style: AppTextStyles.cardSubTitle),
                                        ...
                                        //                 ),
                                        //               )),
                                        //               AppNewTagButton(title: _relateEmployeeIds[index].classification??"")
                                        //             ],
                                        //           ),
                                        //         ),
                                        //       );
                                                        ...
                                                        Column(
                                                          ...
                                                          children: [
                                                            CustomFormField(title:  fields.getField('latitude').label,value: _mosqueGeoLocation.latitude,
                                                                ...
                                                                onChanged:(val){
                                                                    _mosqueGeoLocation.latitude= double.parse(val??"0.0");
                                                            ...
                                                            CustomFormField(title:  fields.getField('longitude').label,value: _mosqueGeoLocation.longitude,
                                                                isRequiredField:requiredFields.contains("longitude"),
                                                                  onChanged:(val){
                                                                    _mosqueGeoLocation.longitude= double.parse(val??"0.0");
                                                                  },isReadonly:true ),
                                                  ...
                                                  children: [
                                                    ...
                                                    Row(
                                                      children: [
                                                        ...
                                                        SecondaryOutlineButton(text: "view_map".tr(),
                                                            icon:AppIcons.mosqueLocation,
                                                            onTab:() async{

                                                              double latitude=_mosque.latitude??0;
                                                              double longitude=_mosque.longitude??0;
                                                              ...
                                                              if (await canLaunchUrlString(googleMapsUrl)) {
                                                                await launchUrlString(googleMapsUrl); // Launch Google Maps
                                                              } else if (await canLaunchUrlString(appleMapsUrl)) {
                                                                await launchUrlString(appleMapsUrl); // Fallback for iOS if Google Maps is not installed
                                                              } else {
                                                                throw 'Could not open Google Maps or Apple Maps';
                                                              }
        ...
        floatingActionButton: (_tabs[_tabController.index].isEditMode==false &&
            _tabs[_tabController.index].isShowEdit==true && (_mosque.state=='draft' || _mosque.state=='edit')
          ...
          onPressed: () async{
            if(isVerifyDevice()) {
              ...
              if (_tabController.index == 0)
                ...
                _tempAllMosqueMeters=[];
                _tempAllWaterMeters =[];
                _allMosqueMeters.forEach((meter) {
                  _tempAllMosqueMeters.add(MosqueMeter.shallowCopy(meter));
                });
                _allWaterMeters.forEach((meter) {
                  _tempAllWaterMeters.add(MosqueMeter.shallowCopy(meter));
                });
              }
              ...
              //Use this code for Swtich button
              Future.delayed(Duration(seconds: 1),(){
                setState(() {
                });
              });
