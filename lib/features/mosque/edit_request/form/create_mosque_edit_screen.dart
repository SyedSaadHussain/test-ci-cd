...
class CreateMosqueEditScreen extends StatefulWidget {
  ...
  const CreateMosqueEditScreen({super.key, this.mosqueId, this.localId, this.createIfMissing = true, this.customLocalLoader, this.onTabOpened // NEW
  });
  @override
  State<CreateMosqueEditScreen> createState() => _CreateMosqueEditScreenState();
}

class _CreateMosqueEditScreenState extends BaseState<CreateMosqueEditScreen>
  ...
  void initState() {
    ...
    vm.onChangeTabs=((){
      ...
      final oldController = vm.tabController;
      // Dispose old safely *before* overwriting
      oldController?.dispose();
      vm.tabController = TabController(length: vm.tabs.length, vsync: this);
      vm.notifyListeners();
      vm.tabController!.animateTo(currentIndex);
      (vm as CreateMosqueEditViewModel).addTabController();
    });
