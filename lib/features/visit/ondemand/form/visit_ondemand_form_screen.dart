...
import 'package:mosque_management_system/core/models/base_state.dart';
 ...
 class VisitOndemandFormScreen extends StatefulWidget {
  final VisitOndemandModel visit;
  final Function? onCallback;
  const VisitOndemandFormScreen({super.key, required this.visit,this.onCallback});

  @override
  State<VisitOndemandFormScreen> createState() => _VisitOndemandFormScreenState();
}

 class _VisitOndemandFormScreenState extends BaseState<VisitOndemandFormScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    final vm = context.read<VisitFormViewModel<VisitOndemandModel>>();
  ...
  Widget build(BuildContext context) {
    final vm = context.read<VisitFormViewModel<VisitOndemandModel>>()  as  VisitOndemandFormViewModel;

    return Scaffold(
      appBar: AppBar(
          title:   Text(VisitMessages.onDemandVisit),
          actions:[
            Container(
              height: 40,
            ...
            Consumer<VisitFormViewModel<VisitOndemandModel>>(
      ...
      body: Stack(
        children: [
                        ...
                        Expanded(
                          child:Column(
                            children: [
                                  ...
                                  child: TabBarView(
                                    ...
                                    children:[
                                      SingleChildScrollView(
                                        child: Consumer<VisitFormViewModel<VisitOndemandModel>>(
                                            builder: (context, vm, _) {
                                              ...
                                              return  ViewBasicInfoSection(visit: vm.visitObj,fields: vm.fields,headersMap: null,
                                                extraFields: [
                                                  AppInputView(title: vm.fields.getField('deadline_date').label,value:  vm.visitObj.deadlineDate,type: ListType.date,isShowWarning:  vm.visitObj.isDeadlineActive==false)
                                                ],
                                              );
                                            }
                                        ),
                                      ),
                                      ImamSection<VisitOndemandModel>(),
                                      MosqueMeterSection<VisitOndemandModel>(),
                                      BuildingSection<VisitOndemandModel>(),
                                      DeviceSection<VisitOndemandModel>(),
                                      SafetySection<VisitOndemandModel>(),
                                      MaintenanceSection<VisitOndemandModel>(),
                                      SecuritySection<VisitOndemandModel>(),
                                      DawahSection<VisitOndemandModel>(),
                                      MosqueStatusSection<VisitOndemandModel>(),
                                    ]
                              ...
                              SafeArea(child: VisitFormBottom<VisitOndemandModel>()),
          ...
          Selector<VisitFormViewModel<VisitOndemandModel>, bool>(
