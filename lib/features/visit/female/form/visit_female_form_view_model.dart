...
class VisitFemaleFormViewModel extends VisitFormSubViewModel<VisitFemaleModel> {
  ...
  @override
  submitVisit(context){
    visitRepository.submitFemaleVisit(visitObj,disclaimerContent).then((result){
      isLoading=false;
      notifyListeners();
  ...
  Future<void> initializeVisit() async{
    ...
    notifyListeners();
    await visitRepository.getInitializeFemaleVisit(visitParam.id!).then((result){
      isLoading=false;
