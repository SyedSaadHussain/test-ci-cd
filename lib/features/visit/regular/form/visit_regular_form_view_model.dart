...
import 'package:mosque_management_system/features/visit/core/viewmodel/visit_form_sub_view_model.dart';
 ...
 class VisitRegularFormViewModel extends VisitFormSubViewModel<RegularVisitModel> {

  /// Hive box used for visit data storage
  HiveBoxHelper visitHiveBox = HiveRegistry.regularVisit;

  /// Path to JSON file containing translation labels and combo data
  @override
  String get viewPath => 'assets/views/visit_regular.json';

  VisitRegularFormViewModel({
  ...
  @override
  submitVisit(context){
    visitRepository.submitRegularVisit(visitObj,disclaimerContent).then((result){
      isLoading=false;
      notifyListeners();
  ...
  Future<void> initializeVisit() async{
    ...
    notifyListeners();
    await visitRepository.getInitializeRegularVisit(visitParam.id!).then((result){
      isLoading=false;
