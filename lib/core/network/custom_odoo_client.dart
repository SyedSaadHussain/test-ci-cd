...
// Singleton instance
  ...
  CustomOdooClient._internal(this.baseURL)
    ...
    if(Config.enableSentry || Config.disableValidation){
      _setupInterceptors();
    }
  }
  ...
  Future<Map<String, dynamic>> post(String path, Map<String, dynamic> params,[bool includeJsonrpc=true]) async {
    try {
      ...
      }else if(AppUtils.isInnerException(response.data)){
        throw OdooException(result['result']['message']);
      }else if(AppUtils.isException(response.data)){
        throw OdooException(result['message']);
      }
  ...
  Future<Map<String, dynamic>> get(String path, [Map<String, dynamic>? queryParams]) async {
    try {
      ...
      if(response.headers['content-type']?.any((a)=>a.contains('text/html'))==true){
        ...
        throw OdooException(response.data['result']['message']);
      }
      else if(AppUtils.isException(response.data)){
        throw OdooException(response.data['message']);
      }
