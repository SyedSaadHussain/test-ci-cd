...
class GPSPermission {
  ...
  static const String ltrMark = '\u200E';

  String get outsideRadiusWithSign {
    if (outsideRadius < 0) {
      return '$ltrMark-${outsideRadius.abs()}$ltrMark';
    } else {
      return '$ltrMark${outsideRadius}$ltrMark';
    }
  }
  ...
  status==GPSPermissionStatus.unAuthorizeLocation?('gps_status_un_authorize_location'.tr())+' ('+outsideRadiusWithSign+')':
  ...
  void getCurrentLocation(){
    ...
    Geolocator.getCurrentPosition().then((value){
      ...
      if(Config.disableValidation==false && value.isMocked){
        status=GPSPermissionStatus.mocked;
        fetchDoneController.add(false);
      }else if(isOnlyGPS){
  ...
  bool isValidLocation(double pram_latitude,double pram_longitude){
    distanceInMeters = Geolocator.distanceBetween(pram_latitude, pram_longitude, this.latitude, this.longitude);
    //distanceInMeters = Geolocator.distanceBetween(currentPosition.latitude, currentPosition.longitude, 24.963330, 67.074874);
  ...
  int getTotalMatch(List<List<double>> coordinates){
    int totalMatch=0;

    for (var coordinate in coordinates) {
      double localLatitude = coordinate[0]; // First element is latitude
      double localLongitude = coordinate[1]; // Second element is longitude
      distanceInMeters = Geolocator.distanceBetween(localLatitude, localLongitude, this.latitude, this.longitude);
      if(distanceInMeters<=allowDistance){
        totalMatch++;
      }
    }
